...
export class ChartService implements OnDestroy  {
  ...
  constructor() {
    this.client = Stomp.over(new SockJS(`${environment.chat.chatUrl}/`));
  ...
  onMessage(topic: string, handler = ChartService.jsonHandler): Observable<any> {
  ...
  onPlainMessage(topic: string): Observable<string> {
    return this.onMessage(topic, ChartService.textHandler);
  }

  onChatMessage(topic: string): Observable<ChatMessage> {
    return this.onMessage(topic, ChartService.jsonHandler);
  }
